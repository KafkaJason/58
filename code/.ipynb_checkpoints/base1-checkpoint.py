...
def create_dir():
    ...
    if not os.path.exists(FEAT_PATH):
        print('Create dir: %s'%FEAT_PATH)
        os.mkdir(FEAT_PATH)
    if not os.path.extsts(RESULT_PATH):
        print('Create dir: %s'%RESULT_PATH)
...
def prepare(train_data, test_data):
    ...
    data_all[['47','48']] = data_all[['47','48']].replace('-1', '')
...
def neg_data(train_data, test_data):
    ...
    feat_1 = [37,102,79,1,4,845]
    
    # train_data = train_data.drop(train_data[train_data['1']>900].index)
    train_data = train_data[train_data['1'].isin(feat_1)]
...
data_all.to_feather(FEAT_PATH + 'data_all_prepare.feather')
...
data_all.to_feather(FEAT_PATH + 'data_all.feather')
...
def get_feat2(data):
    ...
    for tag in ['47','48']:
        pivot = get_vec(pivot, tag, emb_size=5)
    for tag in ['7','88']:
        pivot = get_vec(pivot, tag, emb_size=200)
    
    pivot.drop(['7','47','48','88'], axis=1, inplace=True)
    pivot.to_feather(FEAT_PATH + 'data_feat1.feather')
...
#     pivot.to_feather(FEAT_PATH + 'test_pivot1.feather')
    ...
    new_data.to_feather(FEAT_PATH + 'data_feat2.feather')
...
def get_feat4(data):
    ...
    pivot.to_feather(FEAT_PATH + 'pivot.feather')
    ...
    new_data.to_feather(FEAT_PATH+'new_data.feather')
...
def get_feat5(new_data):
    ...
    pivot = pd.read_feather(FEAT_PATH + 'pivot.feather')
    data_feat = pivot[['uid']]
    ...
    data_feat.to_feather(FEAT_PATH + 'data_feat3.feather')


new_data = pd.read_feather(FEAT_PATH+'new_data.feather')
get_feat5(new_data)
...
def data_merge():
    ...
    data_all = pd.read_feather(FEAT_PATH + 'data_all.feather')
    data_feat = pd.read_feather(FEAT_PATH + 'data_feat1.feather')
    data_all = pd.concat([data_all, data_feat], axis=1)

    data_feat = pd.read_feather(FEAT_PATH + 'data_feat2.feather')
    ...
    data_feat = pd.read_feather(FEAT_PATH + 'data_feat3.feather')
...
result = pd.DataFrame()
result['ctr'] = (test_click['lgb_click'] + test_click['cat_click'] + test_click['xgb_click']) / 3
result['cvr'] = (test_resume['lgb_resume'] + test_resume['cat_resume'] + test_resume['xgb_resume']) / 3
